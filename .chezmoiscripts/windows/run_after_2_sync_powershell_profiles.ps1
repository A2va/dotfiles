...
if (-Not $mydocuments.ToLower().StartsWith($userprofile_documents.ToLower())) {
    ...
    Copy-Item "$userprofile_documents\WindowsPowerShell\internal.ps1" -Destination "$mydocuments\WindowsPowerShell\" -Recurse
    Copy-Item "$userprofile_documents\WindowsPowerShell\internal.ps1" -Destination "$mydocuments\PowerShell\" -Recurse
}